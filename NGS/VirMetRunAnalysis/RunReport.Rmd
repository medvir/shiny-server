...
t_info = rbind(t_info, data.frame(key = "Quality-filtered reads (DNA/RNA)", value = paste0(round(filtered_reads_dna/1000000, 3), "/", round(filtered_reads_rna/1000000, 3), " M")))
t_info = rbind(t_info, data.frame(key = "Report date", value = factor(Sys.Date())))
...
[^1]: definition of relevant in that context
[^2]: false annotated references

\newpage

## Rejected[^3] Virus species
```{r, echo = FALSE}
